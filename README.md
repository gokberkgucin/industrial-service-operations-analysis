# Industrial Service Operations Analysis

Bu repo, `EK-10_Siemens_Staj_Makale_Analizleri.docx` dosyasından üretilen staj defterinin GitHub Pages için hazırlanmış sayfa sadakati yüksek yayın sürümüdür.

Amaç, staj defterini yeniden yorumlamak değil; kaynak belgenin sayfa sırasını ve görsel düzenini mümkün olduğunca koruyarak gezilebilir, aranabilir ve indirilebilir bir web yayını oluşturmaktır.

## Site

GitHub Pages bağlantısı: https://gokberkgucin.github.io/industrial-service-operations-analysis/

Yerel önizleme:

```powershell
python -m http.server 8765 -d docs --bind 127.0.0.1
```

Ana giriş: [docs/index.html](docs/index.html)

## Kaynak Doğrusu

Canonical source: `EK-10_Siemens_Staj_Makale_Analizleri.docx`.

Public yayında sayfa görselleri birincil katmandır. Transcript ve arama katmanı yalnızca erişilebilirlik ve gezinme desteği için ikincil katman olarak tutulur.

## Telif Sınırı

Staj defteri PDF/DOCX çıktısı ve sayfa görselleri bu yayın yüzeyindedir. Üçüncü taraf akademik makale PDF'leri public tracked dosya yapılmaz; yalnızca bibliyografik kayıt ve DOI/yayıncı bilgisi verilir.

Ayrıntı: [COPYRIGHT_NOTES.md](COPYRIGHT_NOTES.md).
